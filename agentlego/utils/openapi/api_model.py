# Copied from https://github.com/langchain-ai/langchain/blob/master/libs/community/langchain_community/tools/openapi/utils/api_models.py  # noqa: E501
# Modified to support multipart/form-data style media-type
...
# See https://github.com/OAI/OpenAPI-Specification/blob/main/versions/3.1.0.md#parameterIn  # noqa: E501
# for more info.
...
class APIPropertyBase(BaseModel):
    ...
    Either a primitive type, a component/parameter type, or an array or
    'object' (dict) of the above.
    """

    format: Optional[str] = Field(alias='format', default=None)
    """The modifier property to provide detail for primitive data types."""
...
class APIProperty(APIPropertyBase):
    ...
    @staticmethod
    def _cast_schema_list_type(schema: Schema,
    ...
    @staticmethod
    def _get_schema_type_for_array(schema: Schema,
                                   ) -> Optional[Union[str, Tuple[str, ...]]]:
    ...
    def _get_schema_type(parameter: Parameter, schema: Optional[Schema]) -> SCHEMA_TYPE:
        ...
        if schema_type == 'array':
            raise NotImplementedError('Array not yet supported')
...
class APIRequestBody(BaseModel):
    ...
    # E.g., application/json or multipart/form
    ...
    @classmethod
    def from_request_body(cls, request_body: RequestBody,
                          spec: OpenAPISpec) -> 'APIRequestBody':
        """Instantiate from an OpenAPI RequestBody."""
        properties = []
        for media_type, media_type_obj in request_body.content.items():
            if media_type not in _SUPPORTED_REQUEST_MEDIA_TYPES:
                continue
...
class APIResponseProperty(APIPropertyBase):
    """A model for a response property."""

    @classmethod
    def from_schema(cls, schema: Schema, name: str,
                    required: bool) -> 'APIResponseProperty':
        """Recursively populate from an OpenAPI Schema."""
        schema_type = schema.type
        if schema_type in ['array', 'object']:
            # Handle the complex type as simple string
            schema_type = 'string'
        elif schema_type in PRIMITIVE_TYPES:
            # Use the primitive type directly
            pass
            ...
            # Handle the unknown type as simple string
            schema_type = 'string'
...
class APIResponse(BaseModel):
    ...
    def _process_supported_media_type(
        ...
        media_type_obj: MediaType,
        spec: OpenAPISpec,
    ) -> List[APIResponseProperty]:
        """Process the media type of the response."""
        from openapi_pydantic import Reference
        ...
        if schema.type == 'object' and schema.properties:
            ...
            for prop_name, prop_schema in schema.properties.items():
                ...
                properties[prop_name] = APIResponseProperty.from_schema(
                    schema=prop_schema,
                    name=prop_name,
                    required=prop_name in required_properties,
                )
        elif schema.type == 'array' and schema.prefixItems:
            # Tuple style output
            properties = []
            for i, prop_schema in enumerate(schema.prefixItems):
                if isinstance(prop_schema, Reference):
                    prop_schema = spec.get_referenced_schema(prop_schema)
...
class APIOperation(BaseModel):
    ...
    def from_openapi_spec(
        ...
        responses = spec.get_responses_for_operation(operation)
        if responses is not None:
            api_responses = {
                k: APIResponse.from_response(response, spec)
    ...
    def _format_nested_properties(self,
                                  properties: List[APIRequestBodyProperty],
                                  indent: int = 2) -> str:
